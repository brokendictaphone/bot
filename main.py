...
cur.execute("""CREATE TABLE IF NOT EXISTS things (
    user_id INT,
    thing TXT
)""") #создание таблицы в базе данных если она ещё не создана
      # в скобках указаны столбцы и тип данных в них

cur.execute("""CREATE TABLE IF NOT EXISTS msg_ids (
    user_id INT,
    msg NULL
)""") #создание таблицы с мессадж айди
      # в скобках указаны столбцы и тип данных в них
...
async def command_start(message : types.Message):
    global FrstMessFlag
    ID = message.chat.id
    frst_name = message.chat.first_name
